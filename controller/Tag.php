...
class TagController extends \Xpmse\Loader\Controller {
	...
	function index() {	
		...
		App::render($data,'tag','search.index');
	...
	function detail() {
		...
		return [
			...
			'crumb' => [
	            "标签" => APP::R('tag','index'),
	            "标签管理" =>APP::R('tag','index'),
	            "$action_name" => ''
	        ],
	        'active'=> [
	 			'slug'=>'xpmsns/pages/tag/index'
	 		]
		];
	...
	function remove(){
		$tag_id = $_POST['tag_id'];
		$inst = new \Xpmsns\Pages\Model\Tag;
		$tag_ids =$inst->remove( $tag_id, "tag_id" );
		echo json_encode(['message'=>"删除成功", 'extra'=>['$tag_ids'=>$tag_ids]]);
	}

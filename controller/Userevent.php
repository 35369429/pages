...
class UsereventController extends \Xpmse\Loader\Controller {
	...
	function index() {	
		...
		$search  = $query = $_GET;
		$inst = new \Xpmsns\Pages\Model\Userevent;
		...
		App::render($data,'userevent','search.index');
	...
	function detail() {
		...
		$userevent_id = trim($_GET['userevent_id']);
		$action_name = '新建报名';
		$inst = new \Xpmsns\Pages\Model\Userevent;
		
		if ( !empty($userevent_id) ) {
			$rs = $inst->getByUsereventId($userevent_id);
			if ( !empty($rs) ) {
				$action_name =  $rs['userevent_id'];
			}
		}
		...
		App::render($data,'userevent','form');
		...
		return [
			...
			'crumb' => [
	            "报名" => APP::R('userevent','index'),
	            "报名管理" =>APP::R('userevent','index'),
	            "$action_name" => ''
	        ],
	        'active'=> [
	 			'slug'=>'xpmsns/pages/userevent/index'
	 		]
		];
	...
	function save() {
		...
		$inst = new \Xpmsns\Pages\Model\Userevent;
		$rs = $inst->saveByUsereventId( $data );
		echo json_encode($rs);
	}
	 ...
	 */
	function remove(){
		$userevent_id = $_POST['userevent_id'];
		$inst = new \Xpmsns\Pages\Model\Userevent;
		$userevent_ids =$inst->remove( $userevent_id, "userevent_id" );
		echo json_encode(['message'=>"删除成功", 'extra'=>['$userevent_ids'=>$userevent_ids]]);
	}
	 ...
	 */
	function duplicate(){
		$userevent_id = $_GET['userevent_id'];
		$inst = new \Xpmsns\Pages\Model\Userevent;
		$rs = $inst->getByUsereventId( $userevent_id );
		$action_name =  $rs['userevent_id'] . ' 副本';

		// 删除唯一索引字段
		unset($rs['userevent_id']);
		unset($rs['event_user_id']);
